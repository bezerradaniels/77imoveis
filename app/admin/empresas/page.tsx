...
export default async function AdminEmpresas({ searchParams }: { searchParams: { q?: string; city?: string } }) {
  const [items, cities] = await Promise.all([
    adminListCompanies({ text: searchParams.q, cityId: searchParams.city }),
    getCitiesAll(),
  ]);
  return (
    <div>
