...
export function Dropdown({
  ...
  return (
    <div ref={ref} className={cn('relative', className)}>
      ...
      {open && (
        <div className="absolute left-0 right-0 top-full z-50 mt-1 overflow-hidden rounded-lg border border-border bg-surface">
          {options.map((o) => (
            <button
              key={o.value}
              type="button"
              ...
              className={cn(
                ...
                o.value === value && 'bg-primary/10 text-link',
              )}
            >
              {o.label}
            </button>
