...
export function CompanyAdmin({ id, status, verified, featured }: { id: string; status: string; verified: boolean; featured: boolean }) {
  ...
  const run = (patch: Record<string, any>) => start(async () => { await adminUpdateCompany(id, patch); router.refresh(); });

  const btn = 'inline-flex items-center gap-1 rounded-md border border-border px-2 py-1 text-xs disabled:opacity-50';
  return (
