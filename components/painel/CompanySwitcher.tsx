  ...
  return (
    <div className="relative">
      <button
        ...
        onClick={() => setOpen((v) => !v)}
        disabled={pending}
        className="flex w-full items-center gap-2 rounded-[10px] border border-border bg-surface px-3 py-2 text-left transition hover:bg-subtle disabled:opacity-60"
      >
        <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-primary/10 text-primary">
          {active ? <Building2 size={16} /> : <User size={16} />}
        </span>
        <span className="min-w-0 flex-1">
          <span className="block truncate text-sm font-bold leading-tight text-text">{label}</span>
      ...
      {open && (
        <>
          ...
          <div className="absolute left-0 right-0 z-50 mt-1 overflow-hidden rounded-xl border border-border bg-surface py-1 shadow-lg">
            <button
              type="button"
            ...
            {companies.map((c) => (
              ...
              >
                ...
                <span className="min-w-0 flex-1">
                  ...
                  <span className="block truncate text-xs text-muted">{companyTypeLabel(c.type)}</span>
                </span>
            ...
            <a
              ...
              className="flex items-center gap-2 border-t border-border px-3 py-2 text-sm font-semibold text-primary hover:bg-subtle"
            >
